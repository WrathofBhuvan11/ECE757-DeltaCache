...
# Copyright (c) 2026 DeltaCache Authors
# All rights reserved.
...
def create_system(
    ...
    if buildEnv["PROTOCOL"] != "DeltaCache":
        fatal(
            "This script requires the DeltaCache protocol to be built."
        )
    ...
    # consistent with the NetDest list. Therefore the cache controller nodes
    # must be listed before directory nodes, directory nodes before DMA nodes,
    # etc.
    #
    l0_cntrl_nodes = []
    l1_cntrl_nodes = []
    ...
    for i in range(options.num_clusters):
        for j in range(num_cpus_per_cluster):
            ...
            l0_cntrl = DeltaCache_L0Cache_Controller(
            ...
            l1_cntrl = DeltaCache_L1Cache_Controller(
                version=i * num_cpus_per_cluster + j,
                cache=l1_cache,
        ...
        for j in range(num_l2caches_per_cluster):
            ...
            l2_cntrl.L1RequestToL2Cache.in_port = (
                ruby_system.network.out_port
            )

            l2_cntrl.responseToL2Cache = MessageBuffer()
            l2_cntrl.responseToL2Cache.in_port = (
                ruby_system.network.out_port
            )

    #
    # Run each of the ruby memory controllers at a ratio of the frequency of
    # the ruby system. The clk_divider value is a fix to pass regression.
    #
    ...
    for i, dma_port in enumerate(dma_ports):
        ...
        dma_cntrl = DeltaCache_DMA_Controller(
            version=i,
    ...
    #
    # Create the IO controller and sequencer for full-system mode.
    #
    if full_system:
        io_seq = DMASequencer(version=len(dma_ports), ruby_system=ruby_system)
        ruby_system._io_port = io_seq

        io_controller = DeltaCache_DMA_Controller(
    ...
    #
    # Register configuration with filesystem in SE mode.
    #
    else:
        for i in range(options.num_clusters):
            for j in range(num_cpus_per_cluster):
